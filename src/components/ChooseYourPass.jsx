    ...
    <section id="tickets" className="py-12 md:py-20 px-4 sm:px-6 lg:px-8 bg-[#2D0A22] text-[#F3EAD9] relative overflow-hidden border-b border-[#D4AF37]/30">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="rounded-3xl overflow-hidden border-2 border-[#D4AF37]/50 shadow-2xl flex flex-col lg:flex-row items-stretch">
          ...
          <div className="w-full lg:w-5/12 relative bg-[#1B0312] flex items-center justify-center p-3 sm:p-4 lg:p-6 border-b lg:border-b-0 lg:border-r border-[#D4AF37]/40 group overflow-hidden shrink-0">
            <div className="w-full max-w-md sm:max-w-lg lg:max-w-none mx-auto flex items-center justify-center">
              <img
                src="/assets/images/royal_pass_ticket_banner.jpg"
                alt="GENZATRI Royal Pass - Your Pass To Unforgettable Nights"
                loading="eager"
                className="w-full h-auto max-h-[480px] sm:max-h-[540px] lg:max-h-none object-contain block select-none rounded-2xl transform transition-transform duration-700 group-hover:scale-[1.02]"
              />
            </div>
          </div>

          {/* Right Column: Deep Royal Maroon Pass Panel (#2D0A22) with Authentic Ticket Cards */}
          <div className="w-full lg:w-7/12 bg-[#2D0A22] relative p-4 sm:p-5 lg:p-6 xl:p-8 flex flex-col justify-center space-y-4 sm:space-y-6 overflow-hidden">


            {/* Header Content */}
            <div className="relative z-10 space-y-1 text-center">
              <h3 className="font-serif-display text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl font-black tracking-wider text-[#F3EAD9] uppercase pt-1">
                CHOOSE YOUR NAVRATRI VIBE
              </h3>
              <p className="text-[10px] sm:text-xs md:text-sm font-extrabold tracking-widest text-[#D4AF37] uppercase">
                More people. More memories.
              </p>
            </div>

            {/* 3 Ticket Passes Grid matching Reference Ticket Layout (Filling Height Elegantly) */}
            <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-2.5 sm:gap-3.5 items-stretch">
              {PASSES.map((pass) => (
                <div
                  key={pass.id}
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col justify-between p-2.5 sm:p-3.5 text-left space-y-1.5 pl-2.5 sm:pl-3">
                    <div className="space-y-1">
                      {/* Sub-label */}
                      <span className="block text-[8.5px] sm:text-[9px] font-bold tracking-[0.16em] text-[#D4AF37]/90 uppercase">
                        {pass.id === 'solo' ? 'INDIVIDUAL' : pass.id === 'couple' ? 'DUO PAIR' : 'TRIBE PASS'}
                      </span>

                      {/* Pass Name */}
                      <h4 className="font-serif-display font-black text-[10px] sm:text-[11px] md:text-xs text-[#F3EAD9] uppercase leading-tight tracking-wide">
                        {pass.name}
                      </h4>

                      {/* Price */}
                      <div className="font-serif text-xl sm:text-2xl md:text-[26px] font-black text-white leading-none tracking-tight pt-0.5">
                        {pass.formattedPrice}
                      </div>

                      {/* Description */}
                      <p className="text-[9px] sm:text-[10px] text-[#EAD0B3]/85 leading-tight font-medium pt-0.5">
                        {pass.description}
                      </p>

                      {/* Perks Highlights */}
                      <div className="pt-1.5 border-t border-[#6E2346]/50 space-y-0.5 text-[8.5px] sm:text-[9px] text-[#F3EAD9]/80 font-medium">
                        {pass.id === 'solo' && (
                          <>
                    ...
                    <div className="pt-1.5">
                      <button
                        ...
                        className="w-full py-1.5 px-2.5 rounded-xl bg-gradient-to-r from-[#F5E096] via-[#D4AF37] to-[#C9A227] text-[#0A060A] text-[10px] sm:text-[11px] font-black tracking-wider uppercase shadow-md hover:brightness-110 transition-all duration-300 transform active:scale-95 cursor-pointer flex items-center justify-center gap-1"
                      >
                        <span>Book Now</span>
